...
const RequestEventCreation: React.FC = () => {
    ...
    useEffect(() => {
        async function fetching() {
            const events = await getRequestDetails(id as string)
            console.log("----------------Fetching----- the events --------------------------------------------------", events)
    ...
    const onSubmit = async (data: FormValues) => {
        ...
        const response = await requestCreateEvent(data,currentEvent._id)
    ...
    useEffect(() => {
        if (currentEvent) {
            ...
            setValue('userId',currentEvent.userId)
        }
    ...
    console.log(" all the cities ss--------------",cities)
    return (
        <>
            <div className="w-full  bg-white border flex items-center flex-col shadow-md ">
                <h1 className=' m-6 w-full flex justify-center text-3xl font-extrabold'>Request Event Creation Form</h1>
                        ...
                        <div className='w-[100%] flex items-center flex-col justify-center'>
                                    ...
                                    <Box sx={{ m: 1, width: '50%', display: 'flex' }}>
                                        <FormControl error={Boolean(errors.state)} sx={{ m: 1, width: '45%', display: 'flex' }}>
                                          ...
                                          {
                                            currentEvent&&<><div>{currentEvent.city} and {selectedCity}</div></>
                                          }
                                        <FormControl error={Boolean(errors.city)} sx={{ m: 1, width: '45%', display: 'flex' }} >
                                    ...
                                    <Box sx={{ m: 1, width: '50%', display: 'flex' }}>
                                        ...
                                        <FormControl sx={{ m: 1, width: '45%', display: 'flex' }} error={Boolean(errors.eventCategory)}>
                                            ...
                                            <Select
                                                labelId="demo-simple-select-label"
                                                id="demo-simple-select"
                                                value={currentEvent.eventCategory?.category || ''}
                                            ...
                                            >
                                                {category && category.map((ele: any) => (
                                                    <MenuItem key={ele.category} value={ele.category}>
                                                        {ele.category}
                                                    </MenuItem>
                                                ))}
                                            </Select>
...
export default RequestEventCreation